$(document).ready(() => {
        ...
        columns: [
            {
                ...
                render : function(data,type,row){
                    return `<a href="dettagli.html?code=${
                        (data)
                    }" style="text-decoration:none; font-weight: 900;">${(data)}</a>`;
                }
            },
            {
                data: "ARTICOLO",
            ...
            },
            {
                ...
                render : function(data,type,row){
                    return `<a href="dettagli.html?code=${
                        (data)
                    }" style="text-decoration:none; font-weight: 900;">Dettagli</a>`;
                }
            },
        ],
    });
    ...
    richiestaAPIGETLOGS(queryGET(getcommessa));

    $("#cerca").click(() => {
        const searchParams = new URLSearchParams(window.location.search);
        const q = {};
        q.codice = document.getElementById("codice").value;
        const newRelativePathQuery =
            "http://50.19.147.177:3000/storico.html" + "?" + queryGET(q);
...
const richiestaAPIGETLOGS = (query) => {
    ...
    function statusPolling() {
        $.ajax({
            type: "GET",
            url: `http://50.19.147.177:3000/api/logs/last`,
        })
            .then((result) => {
...
const stampaTabella = (result) => {
    /* const template = $(`
        <tr>
        <td><a href="storico.html?code=${
            result.CODICE
        }" style="text-decoration:none; font-weight: 900; font-size: 18px;">${
        result.CODICE
    }</a></td>
        <td style="font-size: 18px;">${result.ARTICOLO}</td>
        <td style="font-size: 18px;">${moment(result.DATAATTUALE).format(
            "DD/MM/YY HH:MM:SS"
        )}</td>
        <td style="font-size: 18px;">${
            result.PEZZI_PRODOTTI_PARZIALI_RELATIVI_COMMESSA
        }</td>
        ...
        <td style="font-size: 18px;">${
            result.TEMPO_DI_PRODUZIONE_MOMENTANEO
        }</td>
        <td style="font-size: 18px;">${result.VELOCITA_MACCHINA_ATTUALE}</td>
        <td style="font-size: 18px;">${result.STATOPROD}</td>
        <td style="font-size: 18px;">${moment(result.DATACONSEGNA).format(
            "DD/MM/YY HH:MM:SS"
        )}</td>
        <td style="font-size: 18px;">${result.STATOMACCHINA}</td>
        <td style="font-size: 18px;">${result.MANUALE}</td>
        <td style="font-size: 18px;"><a href="dettagli.html?code=${
            result.CODICE
        }" style="text-decoration:none; font-weight: 900;">Dettagli</a></td>
        </tr>
    `);*/
