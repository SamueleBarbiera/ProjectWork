...
$(document).ready(() => {
    ...
    const codice = urlParams.get("code");
    richiestaAPIGET(codice);
});

const richiestaAPIGET = (codice) => {
    setInterval(statusPolling, 1000);
    function statusPolling() {
        $.ajax({
            type: "GET",
            url: host + `/commesse/${codice}/details`,
        })
            .then((result) => {
                console.log(result);
                $("#status").empty();
...
const controlloStatus = (result) => {
    ...
    if (result.STATOPROD == "Inattiva") {
        status(false);
    }
    if (result.STATOPROD == "Errore") {
        status(false);
    }
    if (result.STATOPROD == "idle") {
        status(false);
    }
    if (result.STATOPROD == "Attiva") {
        status(true);
    }
};
...
const stampaAvviso = (statop) => {
    ...
    if (statop == "" || statop == "Errore" || statop == "Inattiva") {
        style = "bg-danger";
    }
    let template = $(`
    <link rel="stylesheet" href="../css/style.css" />
    <div class="jumbotron ${style} bggg text-center">
    <h1 class="text-center" style="color:white; font-size:128px;">${statop}</h1>
    </div>   
`);
